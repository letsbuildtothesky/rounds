import 'dart:io';

import 'package:flutter_test/flutter_test.dart';
import 'package:rounds_driver_harness/src/storage/harness_database.dart';
import 'package:rounds_driver_harness/src/storage/pod_evidence_outbox.dart';
import 'package:sqflite_common_ffi/sqflite_ffi.dart';

void main() {
  sqfliteFfiInit();

  test(
    'locally saved POD evidence survives process restart with resume metadata',
    () async {
      final directory = await Directory.systemTemp.createTemp(
        'rounds-pod-outbox-',
      );
      final databasePath = '${directory.path}/pod.db';
      try {
        var database = await databaseFactoryFfi.openDatabase(
          databasePath,
          options: OpenDatabaseOptions(
            version: 1,
            onCreate: (database, _) =>
                HarnessDatabase.createPodEvidenceSchema(database),
          ),
        );
        var outbox = PodEvidenceOutbox(database);
        final saved = await outbox.saveLocal(
          stopId: 'stop-1',
          expectedStopVersion: 5,
          manifestId: 'manifest-1',
          manifestVersion: 1,
          confirmedLineNumbers: const [1, 2],
          localPath: '/private/evidence.jpg',
          sha256: List.filled(64, 'a').join(),
          byteSize: 2048,
          contentType: 'image/jpeg',
          handoffType: 'recipient',
          receiverName: 'Siriporn',
        );
        await outbox.markPrepared(saved.id, {
          'mediaAssetId': 'asset-1',
          'bucket': 'pod-evidence',
          'path': 'tenant/photo.jpg',
          'tusEndpoint': 'https://storage/upload/resumable',
          'uploadAuthorization': 'driver_session',
          'assetState': 'staged',
        });
        await outbox.markUploadUrl(
          saved.id,
          'https://storage/upload/resumable/upload-1',
        );
        await outbox.markOffset(saved.id, 1024);
        await database.close();

        database = await databaseFactoryFfi.openDatabase(databasePath);
        outbox = PodEvidenceOutbox(database);
        final restored = (await outbox.pending()).single;
        expect(restored.localPath, '/private/evidence.jpg');
        expect(restored.uploadUrl, endsWith('upload-1'));
        expect(restored.uploadOffset, 1024);
        expect(restored.receiverName, 'Siriporn');
        final reset = await outbox.resetUploadSession(restored.id);
        expect(reset.status, 'prepared');
        expect(reset.uploadUrl, isNull);
        expect(reset.uploadOffset, 0);
        await database.close();
      } finally {
        await directory.delete(recursive: true);
      }
    },
  );
}

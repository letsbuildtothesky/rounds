...
function ExceptionDrawer({ item, accessToken, tenant, onCommunications, onResolved }: { item: OperationsActionException; accessToken?: string; tenant: OperationsTenant; onCommunications: (threadId?: string) => void; onResolved: () => void }) {
  ...
  const canResolve = Boolean(accessToken && tenant.role !== "viewer" && item.stage === "pickup" && item.stopState === "exception" && (item.roundState === "approved" || item.roundState === "loading"));

  async function resolveException() {
    if (!accessToken || !canResolve || !note.trim()) return;
    setSubmitting(true); setResolutionError("");
    try {
      const response = await fetch(`${roundsApiUrl}/v1/operations/exceptions/${item.id}/resolve`, {
        method: "POST",
        ...
        body: JSON.stringify({ stopId: item.stopId, expectedStopVersion: item.stopVersion, resolution: "pickup_corrected", note: note.trim() }),
      });
  ...
  return <><section className="v45-decision"><small>NEXT DECISION</small><h3>Review {exceptionLabels[item.category].toLowerCase()} report.</h3><p>{item.note || "The driver reported an item problem without an additional note."}</p><div><span><small>Stage</small><b>{item.stage}</b></span><span><small>Reported</small><b>{shortTime(item.reportedAt, tenant.timezone)}</b></span><span><small>State</small><b>Action</b></span></div></section><section className="v45-detail"><h4>Delivery truth <span>realtime</span></h4><dl><div><dt>Round</dt><dd>{item.roundReference}</dd></div><div><dt>Driver</dt><dd>{item.driverName}</dd></div><div><dt>Stop</dt><dd>{item.stopSequence} · {item.stopState} · v{item.stopVersion}</dd></div><div><dt>Manifest</dt><dd>Version {item.manifestVersion}</dd></div></dl></section>{resolving && <section className="v45-resolution"><small>AUDITED RESOLUTION</small><h4>Confirm the pickup issue is corrected</h4><p>This releases the Stop back to <b>Assigned</b>. The driver must physically re-check the manifest before custody transfers.</p><label>Operations evidence note<textarea autoFocus value={note} maxLength={500} onChange={(event) => setNote(event.target.value)} placeholder="What was corrected, checked, and by whom?" /></label>{resolutionError && <div role="alert">{resolutionError}</div>}<div><button type="button" onClick={() => { setResolving(false); setResolutionError(""); }}>Cancel</button><button type="button" className="primary" disabled={submitting || !note.trim()} onClick={() => void resolveException()}>{submitting ? "Committing…" : "Confirm corrected & resume pickup"}</button></div></section>}<div className="v45-drawer-actions">{!resolving && canResolve && <button className="primary" type="button" onClick={() => setResolving(true)}>Resolve corrected pickup</button>}<button disabled={!item.operationsThreadId} onClick={() => onCommunications(item.operationsThreadId)}>Message driver</button>{!canResolve && <small>{item.stage !== "pickup" ? "This exception type needs a separate resolution command." : tenant.role === "viewer" ? "Viewer access is read-only." : "Refresh before resolving this Stop."}</small>}</div></>;
}

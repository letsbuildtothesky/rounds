...
function DriverRow({ driver, tenant, onSchedule, onOpenRound, onCommunications }: { driver: OperationsDriverCapacityItem; tenant: OperationsTenant; onSchedule: () => void; onOpenRound: (roundId: string) => void; onCommunications: () => void }) {
  return <article className="v45-driver-row">
    ...
    <div className="v45-driver-cell"><small>AVAILABILITY</small><b className={`availability-${driver.availability.state}`}>{driver.availability.label}</b><span>{driver.availability.nextAvailableAt ? `Projected ${timeLabel(driver.availability.nextAvailableAt, tenant.timezone)} · ` : ""}{driver.availability.projectionBasis}</span></div>

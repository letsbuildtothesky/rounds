...
function LiveChangeSheet({ stop, stopCount, draft, preview, busy, error, timezone, onChange, onClose, onPreview, onApply }: {
  ...
  </aside></div>{pinPickerOpen && <DestinationPinPicker mode="change" initial={{ latitude: Number(draft.latitude), longitude: Number(draft.longitude) }} onCancel={() => setPinPickerOpen(false)} onConfirm={(coordinate) => { onChange({ ...draft, latitude: String(coordinate.latitude), longitude: String(coordinate.longitude), keepCurrentPin: false, pinSelectionMade: true }); setPinPickerOpen(false); }} />}</>;
}

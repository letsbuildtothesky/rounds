...
export async function reportDriverEmergencyHandler(
  ...
  let payload: ReportDriverEmergencyPayload;
  try {
    payload = await request.json() as ReportDriverEmergencyPayload;
  } catch {
  ...
  try {
    validateReportDriverEmergencyCommand(command);
  } catch (error) {
    return json(422, { error: {
      code: "VALIDATION_FAILED",
      message: error instanceof ContractError ? error.message : "Driver emergency command is invalid",
    } }, traceId);
  }
  const result = await dependencies.stops.reportDriverEmergency(command, identity);

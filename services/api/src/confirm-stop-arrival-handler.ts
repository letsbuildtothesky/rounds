...
export async function confirmStopArrivalHandler(
  ...
  let payload: ConfirmStopArrivalPayload;
  try {
    payload = await request.json() as ConfirmStopArrivalPayload;
  } catch {
  ...
  try {
    validateConfirmStopArrivalCommand(command);
  } catch (error) {
    return json(422, { error: {
      code: "VALIDATION_FAILED",
      message: error instanceof ContractError ? error.message : "Arrival command is invalid",
    } }, traceId);
  }
  const result = await dependencies.stops.confirmStopArrival(command, identity);

import type { OperationsDeliveriesDependencies } from "./types.js";
import { bearerToken, json } from "./http.js";

export async function operationsDeliveriesHandler(
  request: Request,
  dependencies: OperationsDeliveriesDependencies,
): Promise<Response> {
  ...
  if (!actor) return json(403, { error: { code: "NOT_AUTHORIZED", message: "Delivery access is not permitted" } }, traceId);
  return json(200, await dependencies.deliveries.getOperationsDeliveries(actor, dependencies.now()), traceId);
}

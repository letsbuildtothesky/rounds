import type { OperationsActionDependencies } from "./types.js";
import { bearerToken, json } from "./http.js";

export async function operationsActionHandler(
  request: Request,
  dependencies: OperationsActionDependencies,
): Promise<Response> {
  ...
  if (!actor) return json(403, { error: { code: "NOT_AUTHORIZED", message: "Action access is not permitted" } }, traceId);
  return json(200, await dependencies.action.getOperationsAction(actor, dependencies.now()), traceId);
}

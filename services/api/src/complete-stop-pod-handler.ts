...
export async function completeStopPodHandler(
  ...
  let payload: CompleteStopPodPayload;
  try {
    payload = await request.json() as CompleteStopPodPayload;
  } catch {
  ...
  try {
    validateCompleteStopPodCommand(command);
  } catch (error) {
    return json(422, { error: {
      code: "VALIDATION_FAILED",
      message: error instanceof ContractError ? error.message : "POD command is invalid",
    } }, traceId);
  }
  ...
  const result = await dependencies.stops.completeStopPod(command, identity);

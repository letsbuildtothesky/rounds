import { ContractError, validateConfirmDeliveryReturnCommand, type ConfirmDeliveryReturnPayload } from "@rounds/contracts";
import type { ConfirmDeliveryReturnDependencies } from "./types.js";
import { bearerToken, json, statusForCommandError } from "./http.js";

type RequestBody = Omit<ConfirmDeliveryReturnPayload, "exceptionId"> & { stopId: string; expectedStopVersion: number };

export async function confirmDeliveryReturnHandler(request: Request, exceptionId: string, dependencies: ConfirmDeliveryReturnDependencies): Promise<Response> {
  ...
  if (!actor || actor.role === "viewer") return json(403, { error: { code: "NOT_AUTHORIZED", message: "Delivery return confirmation is not permitted" } }, traceId);
  let body: RequestBody;
  try { body = await request.json() as RequestBody; }
  catch { return json(400, { error: { code: "VALIDATION_FAILED", message: "Request body must be JSON" } }, traceId); }
  const command = {
    schemaVersion: 1 as const, commandType: "operations.confirm_delivery_return" as const,
    ...
    payload: { exceptionId, note: body.note },
  };
  try { validateConfirmDeliveryReturnCommand(command); }
  catch (error) { return json(422, { error: { code: "VALIDATION_FAILED", message: error instanceof ContractError ? error.message : "Delivery return confirmation is invalid" } }, traceId); }
  const result = await dependencies.action.confirmDeliveryReturn(command, actor);

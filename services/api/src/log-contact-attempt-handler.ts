...
export async function logContactAttemptHandler(
  request: Request,
  stopId: string,
  ...
  let payload: LogContactAttemptPayload;
  try {
    payload = await request.json() as LogContactAttemptPayload;
  } catch {
    return json(400, { error: { code: "VALIDATION_FAILED", message: "Request body must be JSON" } }, traceId);
  }
  const command = {
    schemaVersion: 1 as const,
    commandType: "stop.log_contact_attempt" as const,
  ...
  try {
    validateLogContactAttemptCommand(command);
  } catch (error) {
    return json(422, { error: {
      code: "VALIDATION_FAILED",
      message: error instanceof ContractError ? error.message : "Contact attempt command is invalid",
    } }, traceId);
  }
  const result = await dependencies.communications.logContactAttempt(command, identity);

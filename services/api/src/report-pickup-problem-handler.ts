...
export async function reportPickupProblemHandler(
  ...
  let payload: ReportPickupProblemPayload;
  try {
    payload = await request.json() as ReportPickupProblemPayload;
  } catch {
    return json(400, { error: { code: "VALIDATION_FAILED", message: "Request body must be JSON" } }, traceId);
  }
  const command = {
    schemaVersion: 1 as const,
    commandType: "stop.report_pickup_problem" as const,
  ...
  try {
    validateReportPickupProblemCommand(command);
  } catch (error) {
    return json(422, { error: {
      code: "VALIDATION_FAILED",
      message: error instanceof ContractError ? error.message : "Pickup problem command is invalid",
    } }, traceId);
  }
  const result = await dependencies.stops.reportPickupProblem(command, identity);

...
| H01 Operations chat | Pilot P1 | `VERIFIED` | The persistent Team Driver ↔ Operations thread works through the server and existing offline message outbox. Its active English shell uses generated measurements from the supplied H01 board, projects truthful sender/system-event states, keeps queued messages visibly local and restores unsent text per Stop. HTTP(S)/`www` URLs are actionable ordinary text and human messages support long-press Copy. Current location is now a real structured attachment: staged before Send, durable across restart/offline, schema/API validated, persisted in the shared thread, rendered on both Driver and Operations, externally map-openable and copyable. | Exercise location stage/send/queue/reconnect/map-open and visual parity on the physical Samsung. Add Camera/Photo/File and voice only with durable local evidence, resumable upload and server thread support; complete unread/read behavior without inventing delivery receipts. |

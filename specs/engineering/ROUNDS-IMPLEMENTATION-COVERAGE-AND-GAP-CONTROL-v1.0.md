...
| H01 Operations chat | Pilot P1 | `VERIFIED` | The persistent Team Driver ↔ Operations text thread works through the server and existing offline message outbox. Its active English shell now uses generated measurements from the supplied H01 board, projects truthful sender/system-event states, keeps queued messages visibly local and restores an unsent text draft per Stop after process restart. Geometry, generated-spec freshness and draft persistence have automated coverage. | Exercise send/queue/reconnect and visual parity on the physical Samsung. Add photo/file/location attachments and voice only with durable local evidence, resumable upload and server thread support; complete unread/read behavior without inventing delivery receipts. |

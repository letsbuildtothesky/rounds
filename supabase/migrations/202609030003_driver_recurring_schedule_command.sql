-- Slice 2: audited recurring own-team schedule configuration.

create or replace function public.set_driver_recurring_schedule_command(
  p_command jsonb,
...
as $$
declare
  v_command_type constant text := 'operations.set_driver_recurring_schedule';
  v_tenant_id uuid;
  v_driver_id uuid;
  v_command_id uuid;
  v_trace_id uuid;
...
begin
  ...
  exception when others then
    return jsonb_build_object('status', 'rejected', 'error', jsonb_build_object(
      'code', 'VALIDATION_FAILED', 'message', 'Schedule identifiers, version or time values are invalid'));
  end;
  ...
  if v_expected_version < 0 or v_idempotency_key = '' or length(v_idempotency_key) > 200
    ...
    or jsonb_array_length(v_payload -> 'weekdays') not between 1 and 7
    or v_start_local = v_end_local or coalesce(length(v_note), 0) > 500 then
    return jsonb_build_object('status', 'rejected', 'error', jsonb_build_object(
      'code', 'VALIDATION_FAILED', 'message', 'Recurring schedule payload is invalid'));
  end if;
  begin
    select array_agg(day_value order by day_value) into v_weekdays
    from (
      select distinct value::smallint as day_value
      from jsonb_array_elements_text(v_payload -> 'weekdays')
    ) days;
  exception when others then
    return jsonb_build_object('status', 'rejected', 'error', jsonb_build_object(
      'code', 'VALIDATION_FAILED', 'message', 'Weekdays must be ISO weekday integers'));
  end;
  if cardinality(v_weekdays) <> jsonb_array_length(v_payload -> 'weekdays')
    or exists (select 1 from unnest(v_weekdays) day where day not between 1 and 7) then
    return jsonb_build_object('status', 'rejected', 'error', jsonb_build_object(
      'code', 'VALIDATION_FAILED', 'message', 'Weekdays must be unique integers from 1 to 7'));
  end if;
  ...
  if not found then
    return jsonb_build_object('status', 'rejected', 'error', jsonb_build_object(
      'code', 'NOT_AUTHORIZED', 'message', 'Driver schedule configuration is not permitted'));
  end if;
  if not exists (
  ...
  select * into v_schedule from public.driver_recurring_schedules
   where tenant_id = v_tenant_id and driver_id = v_driver_id for update;
  if found then
    if v_schedule.version <> v_expected_version then
      return jsonb_build_object('status', 'rejected', 'error', jsonb_build_object(
        'code', 'STALE_VERSION', 'message', 'Driver schedule changed; refresh before saving'));
    end if;
    update public.driver_recurring_schedules
       set weekdays = v_weekdays, start_local = v_start_local, end_local = v_end_local,
           timezone = v_timezone, vehicle_profile_id = v_vehicle_profile_id,
           note = v_note, active = true, version = version + 1,
           updated_at = v_occurred_at, deleted_at = null
     where id = v_schedule.id
     returning id, version into v_schedule_id, v_schedule_version;
  else
    if v_expected_version <> 0 then
      return jsonb_build_object('status', 'rejected', 'error', jsonb_build_object(
        'code', 'STALE_VERSION', 'message', 'Driver schedule does not exist; refresh before saving'));
    end if;
    insert into public.driver_recurring_schedules (
      tenant_id, driver_id, weekdays, start_local, end_local, timezone,
      vehicle_profile_id, note, active
    ) values (
      v_tenant_id, v_driver_id, v_weekdays, v_start_local, v_end_local, v_timezone,
      v_vehicle_profile_id, v_note, true
    ) returning id, version into v_schedule_id, v_schedule_version;
  end if;

  v_state := jsonb_build_object(
    'scheduleId', v_schedule_id, 'driverId', v_driver_id,
    'weekdays', to_jsonb(v_weekdays),
    'startLocal', to_char(v_start_local, 'HH24:MI'),
    'endLocal', to_char(v_end_local, 'HH24:MI'),
    'vehicleProfileId', v_vehicle_profile_id, 'updatedAt', v_occurred_at);
  v_event := jsonb_build_object(
    'event', 'operations.driver_recurring_schedule_set', 'version', 1,
    'eventId', v_event_id, 'traceId', v_trace_id, 'tenantId', v_tenant_id,
    'aggregateType', 'driver_schedule', 'aggregateId', v_driver_id,
    'aggregateVersion', v_schedule_version, 'occurredAt', v_occurred_at,
    'payload', v_state);
  insert into public.audit_events (
    ...
    'operations.driver_recurring_schedule_set', 'driver_schedule', v_driver_id,
    v_schedule_version, v_command_id, v_trace_id,
    jsonb_build_object(
      'scheduleId', v_schedule_id, 'weekdays', to_jsonb(v_weekdays),
      'startLocal', to_char(v_start_local, 'HH24:MI'),
      'endLocal', to_char(v_end_local, 'HH24:MI'),
  ...
  insert into public.domain_event_outbox (
    ...
    v_event_id, v_tenant_id, 'operations.driver_recurring_schedule_set', 1,
    'driver_schedule', v_driver_id, v_schedule_version, v_trace_id, v_event, v_occurred_at);
  v_result := jsonb_build_object(
    'status', 'committed', 'aggregateVersion', v_schedule_version,
...
revoke all on function public.set_driver_recurring_schedule_command(jsonb, uuid)
  from public, anon, authenticated;
grant execute on function public.set_driver_recurring_schedule_command(jsonb, uuid) to service_role;

comment on function public.set_driver_recurring_schedule_command(jsonb, uuid) is
  'Server-only, versioned and audited recurring schedule command for an active own-team driver.';

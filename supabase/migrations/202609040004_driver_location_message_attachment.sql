-- H01 location attachment vertical slice. Location is structured thread data;
-- it is not flattened into message text and does not depend on media retention.

alter table public.operations_messages
  add column attachments jsonb not null default '[]'::jsonb;

alter table public.operations_messages
  drop constraint operations_messages_body_check;

alter table public.operations_messages
  add constraint operations_messages_content_check check (
    jsonb_typeof(attachments) = 'array'
    and jsonb_array_length(attachments) <= 8
    and length(btrim(body)) <= 2000
...
begin
  ...
  if v_expected_version < 1 or v_idempotency_key = '' or length(v_idempotency_key) > 200
    ...
    or exists (
      ...
      where attachment ->> 'kind' <> 'location'
        or length(btrim(coalesce(attachment ->> 'label', ''))) not between 1 and 120
        or jsonb_typeof(attachment -> 'latitude') <> 'number'
        or (attachment ->> 'latitude')::numeric not between -90 and 90
        or jsonb_typeof(attachment -> 'longitude') <> 'number'
        or (attachment ->> 'longitude')::numeric not between -180 and 180
        or (attachment ? 'accuracyMeters' and (
          jsonb_typeof(attachment -> 'accuracyMeters') <> 'number'
          or (attachment ->> 'accuracyMeters')::numeric < 0
        ))
        or coalesce(attachment ->> 'capturedAt', '') !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T'
    ) then
    return jsonb_build_object('status', 'rejected', 'error', jsonb_build_object(
  ...
  v_message := jsonb_build_object(
    'id', v_message_id, 'sender', 'driver', 'body', v_body,
...
comment on column public.operations_messages.attachments is
  'Structured H01 attachments. The first released kind is current location; media kinds remain gated on durable evidence retention.';

create table public.phase_zero_ingest_requests (
  ...
  duplicate_samples integer not null check (duplicate_samples >= 0),
  ingest_watermark bigint not null check (ingest_watermark >= 0),
  broadcast_count integer not null default 1 check (broadcast_count >= 0)
);

create index phase_zero_ingest_requests_tenant_received_idx
  on public.phase_zero_ingest_requests (tenant_id, received_at desc);

alter table public.phase_zero_ingest_requests enable row level security;
...
begin
  ...
  select coalesce(ingest_watermark, 0)
...
revoke all on function public.ingest_phase_zero_location_batch(jsonb) from public;
grant execute on function public.ingest_phase_zero_location_batch(jsonb) to service_role;

create or replace function public.phase_zero_telemetry_snapshot(p_tenant_id uuid)
returns jsonb
language sql
security definer
  ...
  select jsonb_build_object(
    ...
    'positions', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'latitude', st_y(current.position::extensions.geometry),
        'longitude', st_x(current.position::extensions.geometry),
        'sourceAt', current.captured_at,
        'receivedAt', current.received_at,
        'accuracyMeters', current.accuracy_meters,
        'source', current.source,
        'ingestWatermark', current.ingest_watermark,
        'freshness', case
          when now() - current.captured_at <= interval '30 seconds' then 'live'
          when now() - current.captured_at <= interval '90 seconds' then 'aging'
          else 'stale'
        end
      ))
      from public.driver_position_current current
      where current.tenant_id = p_tenant_id
    ), '[]'::jsonb),
    'metrics', jsonb_build_object(
      'requests', (
        select count(*)
        from public.phase_zero_ingest_requests request
        where request.tenant_id = p_tenant_id
      ),
      'ingestRequestsPerSecond', (
        select count(*)::double precision / 60.0
        from public.phase_zero_ingest_requests request
        where request.tenant_id = p_tenant_id
          and request.received_at >= now() - interval '60 seconds'
      ),
      'averageSamplesPerRequest', coalesce((
        select avg(request.sample_count)::double precision
        from public.phase_zero_ingest_requests request
        where request.tenant_id = p_tenant_id
      ), 0),
      'broadcastsPerSecond', (
        select coalesce(sum(request.broadcast_count), 0)::double precision / 60.0
        from public.phase_zero_ingest_requests request
        where request.tenant_id = p_tenant_id
          and request.received_at >= now() - interval '60 seconds'
      )
...
revoke all on function public.phase_zero_telemetry_snapshot(uuid) from public;
grant execute on function public.phase_zero_telemetry_snapshot(uuid) to service_role;
